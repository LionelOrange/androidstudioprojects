...
public class DBService {
    ...
    public static final String sql="insert into question(question,answerA,answerB,answerC,answerD,answer,explaination)values(?,?,?,?,?,?,?)";
    public DBService(Context mContext){
        dbHelper=new MyDatabaseHelper(mContext,"question.db",null,1);
        db=dbHelper.getWritableDatabase();
        if(db.rawQuery("select * from question",null).getCount()<1) {
            db.execSQL(sql,new String[]{"下面关于java.lang.Exception类的说法正确的是（）","继承自Throwable"
            ...
            db.execSQL(sql,new String[]{"下列说法正确的有（）","class中的constructor不可省略","constructor必须与class同名，但方法不能与class同名","constructor在一个对象被new时执行"
                    ,"一个class只能定义一个constructor","2","这里可能会有误区，其实普通的类方法是可以和类名同名的，和构造方法唯一的区分就是，构造方法没有返回值。"});
            db.execSQL(sql,new String[]{"下列属于关系型数据库的是（）"," Oracle"," MySql","IMS","MongoDB"
                    ,"0","非关系型数据库有MongoDB、memcachedb、Redis等"});
            db.execSQL(sql,new String[]{"下列说法正确的是（）","LinkedList继承自List","AbstractSet继承自Set","HashSet继承自AbstractSet","WeakMap继承自HashMap"
                    ,"2","HashSet继承自AbstractSet"});
        }
    }
